const scriptURL =
  "https://script.google.com/macros/s/AKfycbz8dmiVNZqqIlGyaBmXUFN6gW1GnNuW_fVj0EBubSK1wnFxZv34tzK9DLTdYG2hfsDG_w/exec";
...
form.addEventListener("submit", async (e) => {
  ...
  newFormData.append("cauple", formData.get("cauple") || "-");
  newFormData.append("listcauple", formData.get("listcauple") || "-");
...
document.addEventListener("DOMContentLoaded", function () {
  const yescauple = document.getElementById("caupleYes");
  const nocauple = document.getElementById("caupleNo");
  const caupleBlock = document.getElementById("caupleDetails");

  function togglecaupleField() {
    if (yescauple.checked) {
      caupleBlock.classList.add("show");
    } else {
      caupleBlock.classList.remove("show");
    }
  }

  yescauple.addEventListener("change", togglecaupleField);
  nocauple.addEventListener("change", togglecaupleField);
  togglecaupleField();
});
...
document.querySelectorAll('input[name="cauple"]').forEach((radio) => {
  radio.addEventListener("invalid", function (e) {
    e.preventDefault();
    document.getElementById("presenceError").classList.add("show");
    return false;
...
// Находим ВСЕ кнопки
const buttons = document.querySelectorAll('.promting-button');

buttons.forEach(button => {
  button.addEventListener('click', function() {
    // Находим родительский блок .promting для этой кнопки
    const promtingContainer = this.closest('.promting');
    
    // Внутри этого родителя находим скрытый блок
    const hiddenBlock = promtingContainer.querySelector('.visually-hidden-promting');
    
    // Переключаем класс open у скрытого блока
    hiddenBlock.classList.toggle('open');
